--[[
    ╔══════════════════════════════════════════════╗
    ║           GOSHA HUB v14.0                    ║
    ║   Anti-Ban TP | Smart Farm | Clean UI       ║
    ╚══════════════════════════════════════════════╝
]]
...
local Config = {
    ...
    SafeTP = true, TargetMob = "Auto", ESPDistance = 2000, TP_Speed = 100,
    AutoQuest = false, QuestNPC = nil, QuestMob = nil
}
...
local function flyTo(hrp, target, duration)
    ...
    task.wait(randFloat(0.05, 0.15)) -- Случайная задержка после полета
end

-- ===== БЕЗОПАСНЫЙ ТП =====
local function safeTP(hrp, targetCF)
    if not Config.SafeTP then
        hrp.CFrame = targetCF + Vector3.new(0, 5, 0)
        return
    end
    -- Плавный полет через верхнюю точку, чтобы избежать подземелий
    local safeTarget = targetCF + Vector3.new(0, 15, 0) -- Поднимаем цель на 15 studs
    local highPoint = CFrame.new(hrp.Position.X, hrp.Position.Y + 120, hrp.Position.Z)
    flyTo(hrp, highPoint, 0.8)
    task.wait(0.2)
...
-- ===== ОСТРОВА (ИСПРАВЛЕННЫЕ КООРДИНАТЫ) =====
local ISLANDS = {
    ...
    {name="Cafe", cf=CFrame.new(-380, 60, 260), sea=2}, -- Поднято на 60
    {name="Kingdom of Rose", cf=CFrame.new(-400, 35, 2000), sea=2}, -- Поднято на 35
...
local function getMyStats()
    local stats = { level="?", beli="?", fragments="?", fruit="?", style="?", race="?", raceV="?", quest="Нет", island="?" }
    local ls = p:FindFirstChild("leaderstats")
    if ls then
        local l = ls:FindFirstChild("Level"); if l then stats.level = l.Value end
        local b = ls:FindFirstChild("Beli") or ls:FindFirstChild("Money"); if b then stats.beli = b.Value end
        local f = ls:FindFirstChild("Fragments"); if f then stats.fragments = f.Value end
    end
    pcall(function()
        local data = p:FindFirstChild("Data")
        if data then
            local f = data:FindFirstChild("Fruit"); if f then stats.fruit = f.Value end
            local s = data:FindFirstChild("Style"); if s then stats.style = s.Value end
            local r = data:FindFirstChild("Race"); if r then stats.race = r.Value end
            local rv = data:FindFirstChild("RaceVersion"); if rv then stats.raceV = rv.Value end
        end
    end)
    if stats.style == "?" and p.Character then
        for _, tool in ipairs(p.Character:GetChildren()) do
            if tool:IsA("Tool") and tool.ToolTip:find("Fighting Style") then stats.style = tool.Name end
        end
    end
    pcall(function()
        local qg = p.PlayerGui:FindFirstChild("Main")
        if qg then
            local q = qg:FindFirstChild("Quest")
            if q and q.Visible then
                local c = q:FindFirstChild("Container")
                if c then
                    local t = c:FindFirstChild("QuestTitle")
                    if t and t:FindFirstChild("Title") then stats.quest = t.Title.Text end
                end
            end
        end
    end)
    ...
    if char and char:FindFirstChild("HumanoidRootPart") then
        local pos = char.HumanoidRootPart.Position
        local closest, minD = "?", math.huge
        for _, isl in ipairs(ISLANDS) do
            local d = (pos - isl.cf.Position).Magnitude
            if d < minD then minD = d; closest = isl.name end
        end
        stats.island = (minD < 500) and closest or "В море"
    end
    return stats
end
...
local State = {
    ...
    AutoBerryFarm = false, AutoQuest = false,
...
local function cleanOrphanESP()
    pcall(function()
        for _, gui in ipairs(game:GetService("CoreGui"):GetDescendants()) do
            if gui:IsA("Highlight") then gui:Destroy() end
        end
    end)
end
...
FarmTab:CreateToggle({ Name = "Auto Quest (Умный)", CurrentValue = false, Flag = "AutoQuest", Callback = function(v) State.AutoQuest = v end })
...
VisualTab:CreateSlider({ Name = "Дальность ESP", Range = {500, 5000}, Increment = 100, Suffix = " studs", CurrentValue = 2000, Flag = "ESPDistance", Callback = function(v) Config.ESPDistance = v end })
...
TeleportTab:CreateToggle({ Name = "Safe TP (Плавный полет)", CurrentValue = true, Flag = "SafeTP", Callback = function(v) Config.SafeTP = v end })
...
local function getPlayerNames()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= p then table.insert(list, plr.Name) end
    end
    return list
end
...
p.CharacterAdded:Connect(function(char)
    ...
    cleanOrphanESP()
    Rayfield:Notify({Title="ESP", Content="Кэш восстановлен после респавна", Duration=3})
end)
...
task.spawn(function()
    ...
    while getgenv().GOSHA_RUNNING do
        ...
        -- AUTO QUEST (УМНЫЙ)
        if State.AutoQuest and now - lastFarm > 5 then
            lastFarm = now
            local level = 1
            pcall(function() level = p.Data.Level.Value end)
            -- Простой поиск NPC по имени квеста, соответствующего уровню (логика зависит от игры, это пример)
            local questNPC
            for _, obj in ipairs(workspace:GetChildren()) do
                if obj:IsA("Model") and obj:FindFirstChildOfClass("Humanoid") and obj.Name:lower():find("quest") then
                    questNPC = obj; break
                end
            end
            if questNPC and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                local npcPos = questNPC:FindFirstChild("HumanoidRootPart")
                if npcPos then
                    safeTP(p.Character.HumanoidRootPart, safePosition(npcPos.CFrame, 5))
                    task.wait(0.5)
                    pcall(function() ReplicatedStorage.Remotes.CommF_:InvokeServer("StartQuest", "BanditQuest", 1) end)
                end
            end
        end
...
Rayfield:Notify({ Title = "Gosha HUB v14.0", Content = "Загружено! Координаты исправлены, логика обновлена.", Duration = 5 })
warn("[Gosha HUB v14.0] Загружено успешно")
